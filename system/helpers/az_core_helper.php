...
defined('BASEPATH') OR exit('No direct script access allowed');   
    ...
    if(!function_exists('az_thousand_separator')){
        function az_thousand_separator($key ='') {
            ...
            $data = number_format($key, 0, $dc_format, $nm_format);          
            return $data;
        }
    }

    if(!function_exists('az_thousand_separator_decimal')){
        function az_thousand_separator_decimal($key ='', $decimal = 2) {
            $ci =& get_instance();
            $ci->load->helper('az_config');
            $dc_format = ',';
            $nm_format = '.';
            ...
            $data = number_format($key, $decimal, $dc_format, $nm_format);          
            return $data;
        }
    }
    ...
    if(!function_exists('az_convert_month')){
     ...
     if(!function_exists('az_convert_day')){
        function az_convert_day($day = '') {
            $arr_day = array(
                'Sunday' => azlang('Sunday'),
                'Monday' => azlang('Monday'),
                'Tuesday' => azlang('Tuesday'),
                'Wednesday' => azlang('Wednesday'),
                'Thursday' => azlang('Thursday'),
                'Friday' => azlang('Friday'),
                'Saturday' => azlang('Saturday'),
            );
    ...
    if(!function_exists('az_full_date')){
        function az_full_date($date = '') {
            $the_date = '';
            if (strlen($date) > 0) {
                $day = Date('d', strtotime($date));
                $month = Date('m', strtotime($date));
                $year = Date('Y', strtotime($date));
                $tmonth = az_convert_month($month);

                $the_date = $day.' '.$tmonth.' '.$year;
            }
    ...
    if(!function_exists('az_css')){
        function az_css() {
            ...
            foreach((array) $css AS $item){
                $str .= '<link rel="stylesheet" href="'.base_url().'assets/plugins/'.$item.'"/>'."\n";
            }

            foreach((array) $app_css AS $item){
                $str .= '<link rel="stylesheet" href="'.base_url().AZAPP.'assets/plugins/'.$item.'"/>'."\n";
            }

            if (isset($ci->az_css_plugin)) {
                foreach ($ci->az_css_plugin as $key => $value) {
                    $str .= '<link rel="stylesheet" href="'.base_url().'assets/plugins/'.$value.'"/>'."\n";
                }
            }
            if (isset($ci->az_app_css_plugin)) {
                foreach ($ci->az_app_css_plugin as $key => $value) {
                    $str .= '<link rel="stylesheet" href="'.base_url().AZAPP.'assets/plugins/'.$value.'"/>'."\n";
                }
            }
