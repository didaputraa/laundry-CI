...
class CI_AZFile extends CI_AZ {
	...
	public function set_max_width($data) {
		return $this->max_width = $data;
	}
	public function set_max_height($data) {
		return $this->max_height = $data;
	}
	...
	function save($insert_id, $idtable, $table, $idpost, $path = '', $filename = '', $encrypt = true) {
		$ci =& get_instance();
		$err_code = 0;
		$err_message = '';
		$data = array();
		...
		if(isset($_FILES[$this->id]['tmp_name'])){
			...
			if (strlen($this->max_width) > 0) {
				$config['max_width'] = $this->max_width;
			}
			if (strlen($this->max_height) > 0) {
