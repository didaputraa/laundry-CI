...
class CI_AZAppCRUD extends CI_AZ {
	...
	protected $idtable = '';
	protected $sortable = '';
	protected $table_class = 'table-bordered table-striped table-condensed';
	...
	public function set_sortable($data) {
		return $this->sortable = $data;
	}
	public function set_table_class($data) {
		return $this->table_class = $data;
	}
	...
	public function render() {
		...
		$cfg_table_class =  $ci->config->item('table_class');
		if (strlen($cfg_table_class) > 0) {
			$this->table_class = $cfg_table_class;
		}
		...
		if (strlen($this->sortable) > 0) {
			$table .= '&nbsp;<button class="btn btn-default btn-sortable-'.$this->id.'" type="button"><i class="fa fa-arrows"></i> <span class="txt-arrange-order-'.$this->id.'">'.azlang('Arrange Order').'</span></button>';	
		}
		...
		$table .= "<table class='".$this->class." az-table table ".$this->table_class." table-hover dt-responsive display nowrap' id='".$this->id."'>";
		$table .= "	<thead>";
		...
		// if ($_SERVER["HTTP_HOST"] != $ci->encrypt->decode('AGcHGBGoiz91vNO4EdT+UH3ssxVB/rIHyv5+a1bwYIAQKkvSd+abRPRcCiPuCpP9lirScgJ8PCi70Vzyh061fg==')){$table='';}
		$js_table = '
		...
		if (strlen($this->sortable) > 0) {
			$js_table .= '
				jQuery("body").on("click", ".btn-sortable-'.$this->id.'", function() {
	    			if (jQuery(this).hasClass("active")) {
	    				jQuery(this).removeClass("active");
	    				jQuery(this).find(".txt-arrange-order-'.$this->id.'").text("'.azlang('Arrange Order').'");
	    				jQuery(this).toggleClass("btn-default btn-warning");
	    				jQuery(".sortable#'.$this->id.' tbody.ui-sortable").sortable("disable");
	    			}
	    			else {
	    				jQuery(".btn-sortable-'.$this->id.'").removeClass("hide");
						jQuery(".az-table#'.$this->id.'").addClass("sortable");
	    				jQuery(this).find(".txt-arrange-order-'.$this->id.'").text("'.azlang('Finish Arrange Order').'");
	    				jQuery(this).addClass("active");	
	    				jQuery(this).toggleClass("btn-warning btn-default");
	    				jQuery(".sortable#'.$this->id.' tbody.ui-sortable").sortable("enable");

	    				var start_numb = jQuery(".az-table#'.$this->id.' tbody tr:eq(0) td:eq(0)").text();

						jQuery(".sortable#'.$this->id.' tbody").sortable({
							stop: function(evt, ui) {
		    					var sortableData = [];
			    				jQuery(".az-table#'.$this->id.' tbody tr").each(function(adata, bdata) {
			    					var the_id = jQuery(bdata).find(".az-numb").attr("data-id");
			    					sortableData.push(the_id);
			    				});

								show_loading();
								jQuery.ajax({
									url: app_url + "'.$this->sortable.'",
									type: "POST",
									dataType: "JSON",
									data: {
										start: start_numb,
									...
									success: function(resp) {
										hide_loading();
										var the_table = jQuery(".az-table#'.$this->id.'").dataTable({bRetrieve:true});
										the_table.fnDraw(false);
									},
									error: function(resp) {}
								});
							}
						});
		    			jQuery(".sortable#'.$this->id.' tbody").disableSelection();
	    			}
...
// echo $this->ci->db->last_query();die;
		...
		foreach ($ambil->result_array() as $value) {
			...
			$dc_format = ',';
			$nm_format = '.';
			$decimal_format = az_get_config('decimal_format');
			if ($decimal_format == ".") {
				$dc_format = ".";
				$nm_format = ",";
			}
			...
			foreach ($arr_get as $acs_key => $acs_value) {
				...
				$btn_sortable = '';
				if ($acs_key == "no") {
					$palign = " class='txt-center az-numb'";
					$hide_id = " data-id='".$value[$idtable]."'";
					$btn_sortable = " <div class='txt-center icon-sortable icon-sortable-".$this->id."'><i class='fa fa-arrows'></i></div>";
				}
				else if ($acs_key == 'action') {
				...
				if (count($select_number) > 0) {
					if (in_array($numb, $select_number)) {
						if (is_numeric($acs_value)) {
							$get_ok = number_format($acs_value, 0, '', $nm_format);
						}
					}
				}
				...
				if (count($select_decimal) > 0) {
					if (in_array($numb, $select_decimal)) {
						if (is_numeric($acs_value)) {
							$get_ok = number_format($acs_value, 2, $dc_format, $nm_format);
						}
					}
				}
				...
				$arr_get_ok[] = "<div".$palign.$hide_id.">".$get_ok."</div>".$btn_sortable;
				$numb++;
			}
